...
int main(){
    ...
    for (const Grade& grade : grades) {
        int grades_student_id = grade.get_student_id();
        float num_grd;
        if (grades_student_id == id) {
            ...
            int j = 0;
            while (j < courses.size() && courses[j].get_id() != grade.get_course_id()) // finding the course while course_id
                j++;
